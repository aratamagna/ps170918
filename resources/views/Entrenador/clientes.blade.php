    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <center><h2>Clientes propios</h2></center>           
                    ...
                    <div class="table-responsive">              		
                       <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered table-condensed" id="example">
                            ...
                            <tbody>
                               ...
                               $aux=0;
                               ?>
                                @foreach($evaluacionbasicas as $evaluacionbasica)
                                 <?php
                                 $usuario=User::where('id','=',$evaluacionbasica->persona->id)->get();
                                 
                                    if($evaluacionbasica->persona->id!=$aux)
                                    {
                                    ?>    
                                    
                                    <tr>
                                        <!--<td>{{ $evaluacionbasica->user->persona->per_rut }}</td>--><!-- no es lo mismo-->
                                        <td>{{ $evaluacionbasica->persona->per_rut }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_nombre }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_apellidop }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_apellidom }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_nac }}</td>
                                        <td>{{ $evaluacionbasica->persona->sexo->sex_nombre }}</td>
                                        @foreach($usuario as $user)
                                        <td>{{ $user->comuna->region->reg_nom }}</td>
                                         <td>{{ $user->comuna->com_nom }}</td> 
                                        @endforeach 
                                        <td>{{ $evaluacionbasica->persona->per_direccion }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_numdirec }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_numpiso }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_fono1 }}</td>
                                        <td>{{ $evaluacionbasica->persona->per_fono2}}</td>
                                        <td>{{ $evaluacionbasica->persona->per_correo }}</td>
                                 
                                
                                        <td>          
                                            @if(Session::has('errors') and $status=='error2' )
                                            <button type="button" class="btn btn-info btn-sm" onClick="editar('{{$evaluacionbasica->user->id}}')" data-toggle="modal" data-target=".bs-exampleeditabierto-modal-lg"><i class="fa fa-pencil-square-o"> Editar</i></button>
                                            @else
                                            <button type="button" class="btn btn-info btn-sm" onClick="editar('{{$evaluacionbasica->user->id}}')" data-toggle="modal" data-target=".bs-exampleeditcerrado-modal-lg"><i class="fa fa-pencil-square-o"> Editar</i></button>
                                            @endif
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-danger" onClick="eliminar('{{$evaluacionbasica->user->id}}')"><i class="icon-trash"> Eliminar</i></button>
                                        </td>
                                    </tr>
                                    
                                 <?php
                                    }
                                    $aux=$evaluacionbasica->persona->id;
                                 ?>
                                @endforeach  	
                            </tbody>
